...
* ManticoreSearch 7.0 или выше с открытым протоколом MySQL (по умолчанию порт 9306). Именно на этих
  версиях работает CI — 7.0.0, 13.11.1 и текущий релиз, — а более старые не проверены, а не заведомо
  сломаны: образов для них в реестре уже нет. Семантическому поиску нужно больше, см. ниже.
...
| `auto_create` | `SCOUT_MANTICORE_AUTO_CREATE` | `true` | создавать индекс при первой записи; при выключенном запись в несуществующий индекс — ошибка |
...
При выключенном `auto_create` запись в отсутствующий индекс приводит к ошибке, причём драйвер сам
спрашивает сервер, есть ли таблица, а не полагается на ответ записи: начиная с версии 29 Manticore
создаёт таблицу по собственной догадке прямо на записи — то есть ровно то, ради чего `auto_create`
и выключают.
...
Кое-что нужно и от сервера. Векторы появились в ManticoreSearch 6.3, а KNN в нём — отдельная
библиотека: сборка без неё принимает `CREATE TABLE`, а затем отвечает `knn library not loaded`.
Что у сервера есть, он говорит сам:
...
Значение имеет `knn` в этом списке — в официальном образе он есть, как и во всех релизах, на
которых CI пакета гоняет векторные тесты.
